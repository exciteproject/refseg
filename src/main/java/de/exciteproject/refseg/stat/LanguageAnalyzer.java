package de.exciteproject.refseg.stat;
...
public class LanguageAnalyzer {
    ...
    public void analyze(File inputDirectory, File outputFile) throws IOException, LangDetectException {
        List<File> textFiles = FileUtils.listFilesRecursively(inputDirectory);

        Map<String, Integer> languageMap = new HashMap<String, Integer>();

        for (File textFile : textFiles) {
            Detector detector = DetectorFactory.create();
            detector.append(FileUtils.readFile(textFile));
            try {
        ...
        for (Entry<String, Integer> languageMapEntry : MapUtils.entriesReverselySortedByValues(languageMap)) {
            bufferedWriter.write(languageMapEntry.getKey() + ": " + languageMapEntry.getValue());
            bufferedWriter.newLine();
        }
